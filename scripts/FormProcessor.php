...
class FormProcessor
{
    ...
    private function _getFormFieldLabel($field, $properties)
    {
        $type = $properties['type'];
        if ($type === 'file') {
            $value = $properties['value'];
        ...
        switch($type) {
            ...
            case 'radio':
                if (is_array($value)) {
                    $label = $field; // label for group elements
                }
                return $label ? $label : '';
            default:
                return $label ? $label : '';
    ...
    private function _formSubmission($form)
    {
        ...
        $to = $form['email']['to'];
        $toCopy = $form['email']['toCopy'];
        $toHiddenCopy = $form['email']['toHiddenCopy'];
        ...
        $sent = $mail->sendMail($emailFrom, $to, $toCopy, $toHiddenCopy, $subject, $message, true, $emailFrom);
...
STYLES;
        ...
        foreach ($sortedFields as $fieldWrapper) {
            ...
            $message .= '<strong>' . $this->_encodeValue(self::_decodeASCII($this->_getFormFieldLabel($fieldWrapper['field'], $fieldWrapper['properties']))) . ':</strong>';
...
class FormMail extends PHPMailer
{
    ...
    public function sendMail($from, $recipient, $toCopy, $toHiddenCopy, $subject, $body, $mode = false, $replyTo = null, $attachment = null)
    {
        ...
        if ($toCopy && parent::addCC($toCopy) === false)
        {
            return false;
        }

        if ($toHiddenCopy && parent::addBCC($toHiddenCopy) === false)
        {
            return false;
        }
